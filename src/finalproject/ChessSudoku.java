...
public class ChessSudoku
{
	...
	public void solve(boolean allSolutions) {
		sudoku(0, 0, allSolutions);
		for(ChessSudoku solution : solutions) {
...
//			}
	...
	private void sudoku(int x, int y, boolean allSolutions) {
		if(!allSolutions && this.solutions.size()==1) return;
		if(x==this.N && y==0) {
			ChessSudoku possible = new ChessSudoku(this.SIZE);
			for(int i=0; i<this.N; i++) {
				for(int j=0; j<this.N; j++) {
					possible.grid[i][j] = this.grid[i][j];
				}
			}
			this.solutions.add(possible);
			return;
		}
		if(this.grid[x][y]==0) {
			for (int i = 1; i <= this.N; i++) {
				...
				this.grid[x][y] = i;
				if (!sudokumove(x, y)) possible = false;
				if (this.knightRule) {
					if (!knightmove(x, y)) possible = false;
				}
				if (this.kingRule) {
					if (!kingmove(x, y)) possible = false;
				}
				if ((this.queenRule && i == this.N)) { //queen is 16 for 44 grid, and 25 for 55 grid
					if (!queenmove(x, y)) possible = false;
				}
				if (possible) {
					sudoku(x + (y + 1) / N, (y + 1) % N, allSolutions);
				}
			}
			this.grid[x][y]=0;
		}
	...
	private boolean sudokumove(int x, int y) {
		//row
		for(int i=0; i<this.N; i++) {
			if(x==i) continue;
			if(this.grid[i][y] == this.grid[x][y]) return false;
		}
		//column
		for(int i=0; i<this.N; i++) {
			if(y==i) continue;
			if(this.grid[x][i] == this.grid[x][y]) return false;
		}
		//SIZE*SIZE box
		for(int i=x/SIZE*SIZE; i<x/SIZE*SIZE+SIZE; i++) {
			for(int j=y/SIZE*SIZE; j<y/SIZE*SIZE+SIZE; j++) {
				if(i==x && j==y) continue;
				if(this.grid[i][j] == this.grid[x][y]) return false;
			}
		}
		return true;
	}
	...
	private boolean knightmove(int x, int y) {
		...
		for(int i=0; i<8; i++) {
			...
			if(newx < 0 || newx >= this.N) continue;
			if(newy < 0 || newy >= this.N) continue;
			if(this.grid[newx][newy] == this.grid[x][y]) return false;
		}
		return true;
	}
	...
	public static void main( String args[] ) throws Exception {
		...
		InputStream in = new FileInputStream("SudokuPuzzles/hard4x4.txt");
